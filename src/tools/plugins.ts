...
interface Manifest{name:string;version:string;description?:string;entry?:string;permissions?:string[]}
...
export async function registerInstalledPlugins(server:McpServer,ctx:ToolContext):Promise<void>{
  for (const m of manifests()) {
    if (!m.entry) continue;
    const dir = path.join(root(), m.name.replace(/[^a-zA-Z0-9._-]/g,'_'));
    const allowed = (m.permissions || []).every(p => !ctx.token.scopes.length || ctx.token.scopes.includes(p));
    if (!allowed) continue;
    try { const mod = await import(pathToFileURL(path.resolve(dir, m.entry)).href); if (typeof mod.register === 'function') await mod.register(server, ctx); } catch { /* a broken plugin must not prevent gateway startup */ }
  }
}
